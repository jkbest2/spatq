...
prepare_data <- function(catch_df, index_df, mesh, fem,
  ...
  dat <- list(catch_obs = catch_df$catch_biomass,
              ...
              IX_w = stats::model.matrix(~ ftime, data = index_df,
                                         contrasts.arg = list(ftime = X_contr)),
              ## X_n = stats::model.matrix(~ 0 + ftime, data = catch_df),
              ## X_w = stats::model.matrix(~ 0 + ftime, data = catch_df),
              ## IX_n = stats::model.matrix(~ 0 + ftime, data = index_df),
              ## IX_w = stats::model.matrix(~ 0 + ftime, data = index_df),

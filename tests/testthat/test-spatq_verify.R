...
map_epsilon1_n_bad <- list(epsilon1_n = matrix(NA,
                                             nrow(pars$epsilon1_n),
                                             ncol(pars$epsilon1_n)))
map_epsilon1_n <- map_epsilon1_n_bad
map_epsilon1_n$log_kappa <- factor(c(1, 2, NA, 3, 4, 5, 6, 7))
map_epsilon1_n$log_tau <- factor(c(1, 2, NA, 3, 4, 5, 6, 7))
...
test_that("Dimension checking works", {
  ...
  expect_true(verify_spatq_dims(dat, pars, map_epsilon1_n))
...
test_that("Map checking works", {
  ...
  expect_true(verify_spatq_map(pars, map_epsilon1_n))
  expect_error(verify_spatq_map(pars, map_epsilon1_n_bad))
